...
pages.forEach(function(page) {
  ...
  // request(optionsPages, function (error, response, body) {
    ...
    el.getElementsByClassName( 'source-title' ); // Live NodeList of your anchor elements
    

    var dataObj = {title: null, rating: null, url: null};

    el.getElementsByClassName( 'source-title' ).each(function(i, element) {
      ...
      makeGetRequest(pageLink).then(queryResult => {
        // if (error) throw new Error(error);
          var $test = $.parseHTML(queryResult);

          $test("a").each(function (i, element) {
            try {
